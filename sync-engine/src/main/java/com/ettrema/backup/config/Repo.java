...
public interface Repo {
     ...
     * @param localRootPath - the full local path of the root currently being
     * processed
     * @param repoName - the remote name of the root
     * @param isScan - if this is being called from a scan. This means that a
     * given file will only be requested once, allowed the repo to accumulate
     * the file size. A repository might also choose to cache information loaded
     * during a scan, but to re-validate cached data if not in a scan
    ...
    /**
     * Use the file meta in a transaction like block. The repository will, if
     * possible, lock the resource so it cannot be changed during this block.
     *
     * @param filePath
     * @param localRootPath
     * @param repoName
     ...
     * @param folderPath
     * @param repoName
     * @return - a list of the child resources, if any, in the folderPath in
     * this repository
     */
     ...
     * @param file
     * @param status
     * @throws UploadException - for a transient error processing the file.
     * Should be retried
     * @throws RepoNotAvailableException - the repository is temporarily
     * unavailable
     * @throws PermanentUploadException - an exception has occurred which cannot
     * be explicitly handler. Do not retry
     *
     */
    void upload(File file, QueueItem item) throws UploadException, RepoNotAvailableException, PermanentUploadException;

    /**
     *
     * @param dest - the physical file to download to. Will be a temp file
     * @param localTarget - the local copy of the file to download. This
     * identifies the repository file to download
     * @param job
     * @param listener
     ...
     * to dest.
     *
     * @param fullPathFrom - the full local path to the resource which should be
     * moved
     * @param dest - the destination location of the file/folder which has been
     * moved
     * @param job
     */
    void move(String fullPathFrom, File dest, QueueItem item) throws RepoNotAvailableException, PermanentUploadException, UploadException;
     ...
     * @param job
     * @param status
     * @throws DeleteException - for a transient error processing the file.
     * Should be retried
     * @throws RepoNotAvailableException - the repository is temporarily
     * unavailable
     * @throws PermanentUploadException - an exception has occurred which cannot
     * be explicitly handler. Do not retry
     * @return true that the file was deleted, false that it was not (ie did not
     * exist)
     */
    boolean delete(File file) throws DeleteException, RepoNotAvailableException, PermanentUploadException;
    ...
    /**
     * Return true if the given file is in an excluded folder for this
     * repository
     *
     * @param child
     * @return
     ...
     * will mean local files may be modified if a later version is detected int
     * the repository
     *
     * @return
     */
    ...
    /**
     * Get state data, ie not user entered configuration, but data which
     * reflects the current runtime state of the repository. This will be
     * persisted seperately from the configuration
     *
     * @param props
     */
    Object getState();
